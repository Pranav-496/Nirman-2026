...
export default function LoginPage() {
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    ...
    try {
      ...
      setError(detail || 'Google authentication failed. Please try email login instead.');
    } finally {
  ...
  const handleGoogleError = (err) => {
    ...
    setError(
      'Google Login failed — this usually means localhost is not registered as an authorized origin in Google Cloud Console. ' +
      'Please use email/password login below, or add http://localhost:5173 to your OAuth Client ID\'s authorized origins.'
    );
  };
    ...
    <div className="flex items-center justify-center min-h-[80vh] px-4 animate-fade-in">
      <div className="card w-full max-w-md p-8 relative overflow-hidden">
        ...
        {googleLoading && (
          <div className="absolute inset-0 bg-bg/80 backdrop-blur-sm z-20 flex items-center justify-center rounded-2xl">
            <div className="flex flex-col items-center gap-3">
              ...
              <p className="text-sm text-fg-2 font-medium">Authenticating with Google...</p>
            </div>
          </div>
        )}
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-fg-3 mt-1">Sign in to access AuthentiFy verification tools</p>
        </div>
        ...
        <div className="relative mb-6">
          ...
          <div className="relative flex justify-center text-xs">
            <span className="bg-bg-2 px-3 text-fg-3 uppercase tracking-wider">Or continue with email</span>
          </div>
        </div>
